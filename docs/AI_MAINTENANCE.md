...
玩家输入框就是一切剧情入口，**不做意图识别**——问客/闲聊都走自然对话（`genChat`）。`chatContext` 注入【客人】块（当前客人在座 + 点菜台词）。AI 在对话里判断客人说出想吃什么时，**由 AI 在行首写「心愿：」+ 客人原话**（AI 提取，不是正则匹配；`extractComment` 用 `startsWith` 剥离心愿行）。`main.js` 闲聊分支把心愿存 `st.guestWishes[guestId]`（原话，说了什么就是什么）+ 小纸条（act=问客）。上菜时 `scoreDish` 第三参传心愿，`wishMatchScore` 匹配：味型 +5 / 技法 +3 / 点名食材 +4，封顶 +10，命中打 sys 提示。改闲聊/评分相关时别丢这条链。

## 酿造系统（苏唐的活计，跨周长期机制）

- 三系基底（`kind: "基底"`）：蜀南大米/鱼定村青稞/麦芽；四种曲（甜酒曲/麦曲/大曲/藏曲）+ 蒸馏器（商店买）
- `BREW_RECIPES` 18 个配方（data.js），明/藏/川酒风：米酒（甜酿 1 周）、青稞酒（藏曲陶罐 1-2 周）、白酒（大曲+蒸馏器 2-3 周）、黄酒（麦曲陈酿 6-10 周）、麦酒
- 下坛一次投料（基底+曲+辅料）→ `st.brewing` 在酿清单 → **跨周结算**（`settleBrewing`，doNext 翻篇时调）→ 出酒 5 杯进 `st.wines`
- **内功催酿**：`brewWeeks` = ceil(基础周数×(1-内功/250))；内功≥50 米酒立等可取（当周出）
- **苏唐酿酒技能** `suSkills.酿酒`（0-100，初始 5，每次出酒 +2）；`brewQuality` = 40 保底 + 星料 + 技能×0.4 + 工序（黄酒陈酿超期更醇）
- **商店基酒**（SHOP_WINES，应急品质固定）：锦官米酒/玉泉青稞酒/马帮烧酒/锦官黄酒
- **米酒配甜**（WINE_DESSERTS）：扣 1 杯米酒+甜料 → 甜点小吃 3 份
- **白酒/黄酒入药**（MEDICINE_HERBS）：酒泡药材 → 药酒（品质=酒×0.6+药材星×12）
- 酿造面板 `openBrew`（命令「酿造」）：配方/基酒/甜点/入药/在酿一览

## 余味大阵仗（米其林四样定星）

- 余味来时上菜变**大阵仗**：玩家做两道（大菜+汤，`st.feast` 依次入席）→ 「佐餐」选小吃+酒水 → 四样齐「开席」（`doFeastServe`）
- **四样各 25%**：大菜 scoreDish / 汤 scoreDish / 小吃 snackScoreOf / 酒水 wineScore（品质 70%+味型 30%，烈酒余味爱+15 斯文客−10）
- 总分 **≥75 → 1 星、≥85 → 2 星、≥95 → 3 星**（`ryuweiGain` 现评现定，tier 只升不降，pts 记历史最高分）；开席 AI 品评（`genFeastReview`）
- 缺一样 = 该项 0 分，三样满分才 75（封顶 1 星）——必须四样齐且样样硬
