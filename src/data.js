...
export const GUESTS = [
  { id: "caidan", name: "才旦", ident: "村长之女", spend: 20, gender: "女",
  ...
  { id: "zhuoma", name: "卓玛", ident: "村里妇人", spend: 24, gender: "女",
    flavor: "xianxiang", tech: "炖", fav: "牦牛腱子肉",
    order: "认老火慢炖——肉要烂，汤要暖。" },
  { id: "huasao", name: "花嫂", ident: "村口摆摊", spend: 16, gender: "女",
  ...
  { id: "baiguniang", name: "大理白族姑娘", ident: "三道茶的", spend: 44, gender: "女",
  ...
  { id: "daijiak", name: "傣家客人", ident: "版纳来的", spend: 40, gender: "女",
    flavor: "qingdan", tech: "烤", fav: "香茅草",
    order: "香茅草拍一拍，烤鱼包肉都行——要那股柠檬香。" },
  // ── 补全口味 + 苏酥 ────────────────────────────────────────────
  { id: "suniang", name: "酸娘子", ident: "醋坊老板娘", spend: 40, gender: "女",
    flavor: "suanla", tech: "炒", fav: "黑风寨苞谷醋",
  ...
  { id: "lasan", name: "辣三娘", ident: "江湖厨娘", spend: 52, gender: "女",
    flavor: "mala", tech: "炒", fav: "熊山花椒",
    order: "麻辣要够劲，麻得跳脚才好，别手软。" },
  { id: "xianbo", name: "鲜伯", ident: "退休老饕", spend: 60,
    flavor: "xianxiang", tech: "炖", fav: "青衣江团鱼",
    order: "要一个鲜字——吊汤见真章，别糊弄老头子。" },
  { id: "tiannan", name: "甜囡", ident: "糖铺小姐", spend: 38, gender: "女",
    flavor: "tian", tech: "蒸", fav: "雪山野蜂蜜",
    order: "甜要甜得干净，蒸的润一点。" },
  { id: "susu", name: "苏酥", ident: "苏唐的姐姐·御姐", spend: 120, gender: "女", sister: true,
  ...
  // ── 西域·丝路书姬 · 珂萝（胡姬·蜜色瓜子脸·珊瑚金八芒星）──────────
  // 茶马道上走来的胡姬通译兼书商，热络话痨，最爱替人解惑，话尾必补一句确认。
  // 流光炫彩：珊瑚金放射流光（区别于余味横向探照灯/拉姆纵向圣光，ui.js keluo-* / style.css）。
  { id: "keluo", name: "珂萝", ident: "西域·丝路书姬", spend: 130, gender: "女", keluo: true,
    flavor: "xianxiang", tech: "炸", fav: "孜然炙羊腿",
    koupi: "话尾爱补一句「……这样可还清楚？」，急了蹦西域腔",
    body: "西域胡姬，蜜色肌肤，瓜子脸，琥珀金瞳，赤金栗色长卷发缀珊瑚珠，金币抹额、薄纱半面；身量高挑、蜂腰丰臀的奔放身段，珊瑚橘金露脐胡舞装、轻纱披帛、金币腰链、脚踝金铃，背一只书箱——丝路通译兼书商，最爱替人解惑",
    order: "客官这一路辛苦——先坐，奴家与你说道说道这西南的吃食门道……这样可还清楚？" },
  // ── 滇池·求索宫算娘 · 绀曲（蓝鲛少女·推理话痨·水波流光自上而下）────
  // 滇池底求索宫逃出来做题的算学怪胎，帮人不收钱但要听她把道理讲完；
  // 流光炫彩：绀青水波自上而下（区别于余味横向/拉姆圣光/珂萝珊瑚放射，ui.js ganqu-* / style.css）。
  { id: "ganqu", name: "绀曲", ident: "滇池·求索宫算娘", spend: 90, gender: "女", ganqu: true,
    flavor: "qingdan", tech: "蒸", fav: "滇池银鱼",
    koupi: "讲道理要一步一步推，讲完补一句「……可算清楚了？」；帮人不收钱",
    body: "滇池蓝鲛少女，瓜子脸，蓝紫大眼，肤色冷白带蓝调；水母头——绀青波卷及肩、发梢如水母伞缘外翻，缀珠贝小饰。绀青露肩短衫，高开叉拖地长裙半透明、绀青渐变碧蓝、裙摆如水波起伏；裙下一条鲛人大尾巴，鳞片幽蓝闪光。求索宫逃出来做题的算学怪胎，想不通的事睡不着，讲道理停不下来",
    order: "客官且慢，容我先推一步：今日吃食，当以……（噼里啪啦算盘响）这道最合算。不收钱，学问本就该白送。" },
...
export const EXP_SCEN_BY_CAT = {
  ...
  劫镖江湖: ["劫镖·黑风截道", "护镖·夜走官道", "劫镖·马帮火并", "截胡·夺回赃物", "江湖·擂台赌菜",
    ...
    "劫贡还民·截下征贡车，顶级食材物归原主",
    "拦路遇劫·同行女子身陷险境"],
  雪山派: ["掌门失踪的悬案", "雪夜内堂师母咳血", "后山采雪莲遇守塔僧", "顶峰绝顶探崖底", "雪崩封山抢生路",
    "商坊失窃查旧账", "练功堂考较剑法", "三十年前绝顶死斗旧闻", "寒毒入体寻暖源", "雪夜送药汤"],
  ...
  商贸: ["行商·走村串户", "以物易物", "赊账收账", "拍卖·头彩竞价", "赶集·摆摊卖菜",
    "订购·包下果园", "预售·时令菜订金", "换季·清仓甩卖", "团购·府衙采买", "尾货·收山货"],
};
