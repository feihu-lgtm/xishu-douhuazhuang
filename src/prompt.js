...
export function dishUser(ctx) {
  return (
    ...
    sec("输出格式", `第一行「菜名：「xxx」」；换行写正文（3-6 段，穿插「」对话与 *心理*，写师兄掌勺、火候、成菜色香味）；再一行「菜单：」约100字（品名+用料+风味，供记入菜单）；再一行「苏唐批：」；最后一行「心情：」一个词（八个里选）。`)
  );
}
...
export function snackUser(ctx) {
  return (
    ...
    sec("现有食材", ctx.invStr) +
    sec("苏唐自决", "你是苏唐，自己决定做什么小吃、用什么料（最多4样）、做几份、品质如何、是什么味型，师兄管不着。") +
    sec("可选味型", `选一个作为这小吃的味型：${FLAVORS.map(f => f.name).join("/")}。`) +
    sec("档位", tierGuide(ctx.suTier || 1, "苏唐手艺")) +
    sec("输出格式", `只输出 JSON：{"made":"成品名","cat":"汤/饭/点心/串/小吃","flavor":"味型名(从可选味型里选)","used":[用掉的食材名,最多4样,须来自现有且够数],"portions":1-6,"quality":0-100,"desc":"约100字描述(品名+用料+风味,供记入菜单)","proc":"苏唐做这道小吃的过程,按手艺档位写细致度","say":"苏唐说的话","mood":"八个心情词之一"}`)
  );
}
...
export function reactionUser(ctx) {
  return (
    ...
    sec("输出格式", `最后一行单独输出「心情：」一个词（苏唐在一旁旁观的心情，八个里选）。`)
  );
}
