...
export function snackUser(ctx) {
  return (
    ...
    sec("输出格式", `先写约 ${ctx.words || 300} 字的苏唐做小吃小剧情，分 3-5 个自然段、段间空一行（用「」对话与 *心理*，写她如何备料、制作、招呼客人；对客人客气，对师兄调情撒娇）；写完另起一行输出 JSON：{"made":"成品名","cat":"汤/饭/点心/串/小吃","flavor":"味型名(从可选味型里选)","used":[用掉的食材名,最多4样,须来自现有且够数],"portions":1-6,"quality":0-100,"desc":"约100字描述(品名+用料+风味,供记入菜单)","note":"苏唐的≤30字小结(供小纸条存档)","say":"苏唐说的话","mood":"八个心情词之一"}`)
  );
}
