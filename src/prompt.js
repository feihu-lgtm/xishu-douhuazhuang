...
export function snackUser(ctx) {
  return (
    ...
    sec("现有食材", ctx.invStr) +
    sec("苏唐自决", "你是苏唐，自己决定做什么小吃、用什么料（最多4样）、做几份、品质如何、是什么味型，师兄管不着。你是店家，对顾客要客气热情、招呼周到；嘴硬心软只在对师兄时，对客人务必友好。") +
    sec("可选味型", `选一个作为这小吃的味型：${FLAVORS.map(f => f.name).join("/")}。`) +
    sec("档位", tierGuide(ctx.suTier || 1, "苏唐手艺")) +
    sec("输出格式", `先写一段约 ${ctx.words || 300} 字的苏唐做小吃小剧情（用「」对话与 *心理*，写她如何备料、制作、招呼客人，对客人客气）；写完另起一行输出 JSON：{"made":"成品名","cat":"汤/饭/点心/串/小吃","flavor":"味型名(从可选味型里选)","used":[用掉的食材名,最多4样,须来自现有且够数],"portions":1-6,"quality":0-100,"desc":"约100字描述(品名+用料+风味,供记入菜单)","note":"苏唐的≤30字小结(供小纸条存档)","say":"苏唐说的话","mood":"八个心情词之一"}`)
  );
}
