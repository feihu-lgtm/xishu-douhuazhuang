...
import { FLAVOR_BY_ID, FLAVORS, TECHNIQUES, starLabel, weekLabel } from "./data.js?v=v3";
import { currentGuest } from "./state.js?v=v3";
...
export function chatContext(st) {
  ...
  return [
    ...
    sec("苏唐与师兄", `苏唐对师兄好感 ${st?.suAff ?? 0}。${stars > 0 ? `店里挂着余味送的${stars}支银簪（一支=一星），苏唐为这份名头得意，熟人面前会显摆两句，也怕砸了招牌。` : "还没拿到银簪，苏唐盼着余味再来——余味是峨眉破戒的任性女侠，年纪轻轻的一流高手，口味刁钻，苏唐跟她斗嘴斗得开心，也从她嘴里听见天下名馆的见闻。别叫她前辈。"}`),
