...
import { FLAVOR_BY_ID, FLAVORS, TECHNIQUES, TECHNIQUE_IDS, ING_BY_NAME, SNACKS, starLabel, CATEGORY_TASK_TYPES, EXPEDITION_TASK_TYPES } from "./data.js?v=v59";
import { NSFW_RULES, MODE_PRIMER_MESSAGES } from "./modePrimer.js";
import { CHECK_DIMS } from "./state.js?v=v59";
...
import { STYLE, tierGuide, tierOfScore, dishUser, snackUser, reactionUser, RYUWEI_VOICE, HEYUXIE_VOICE } from "./prompt.js?v=v59";
  ...
  if (cfgReady(cfg)) {
    const user = [
      ...
      ctx.guestList && ctx.guestList.length
        ...
        : "",
      `【常客分寸】常客可以自然勾连，但食评人余味不得主动出场、路过或搭话——银簪与名声可以提，人不要写进现场；除非玩家钦定主线点名要她。`,
      `师兄（武功约 ${ctx.skillAvg}、凭平日见识与智慧）与苏唐（手艺 ${ctx.suAvg}）同行，寻稀有食材。`,
      `只输出一个 JSON：{"narrative":"约500字（±10%）第三人称主叙事，3-5段，师兄化解阻碍、苏唐辨认得手，穿插「」对话与*心理*，收尾回店","comment":"苏唐批一句","mood":"八个心情词之一(开心/悠闲/兴奋/心动/得意/不满/吃惊/专注)","special":[{"name":"高级带星食材名，武侠/市井感","stars":1-3,"desc":"一句","label":"食材或酒"}]}`,
      `special 给 1-2 种，种类要多样：肉/河鲜/野果/药材/菌菇/酒/主食等都要有机会，别全是药材菌菇。label 标「酒」的是酒类收获（山野陈酿、窖藏老酒、果酒药酒都行），不标默认食材；酒类的 desc 写清风味、来历与劲道。全部放进同一个 JSON 对象，不要 JSON 之外的多余文字。`,
    ].filter(Boolean).join("\n");
    const t0 = Date.now();
    try {
