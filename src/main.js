// 西蜀豆花庄 · 主循环
import { ING_BY_NAME, RECIPES, INGREDIENTS, starOf, starLabel, EXPEDITION_MAP, EXP_SCEN_BY_CAT, RIVAL_SCHOOLS, GUESTS, TECHNIQUES, FLAVOR_BY_ID, calendarContextFor, weekLabel, RESCUE_SCENARIOS, FEMALE_GUEST_IDS, BREW_RECIPES, SHOP_WINES, WINE_DESSERTS, MEDICINE_HERBS, WORLD_LOCATIONS } from "./data.js?v=v60";
...
import { chatContext } from "./prompt.js?v=v60";
import {
...
const handlers = {
  ...
  sit: () => showInvite(),       // 邀请闲坐：请客坐坐（常客/江湖 留坐闲聊）
...
async function guestArrives() {
  ...
  try {
    ...
    if (g.ryuwei) {
      ...
      keluoIntro(g);  // 西域·珂萝 · ✶ 珊瑚金出场
    } else if (g.ganqu) {
      ganquIntro(g);  // 滇池·绀曲 · ♆ 算娘出场
    }
    else if (isJianghu) {
...
async function cookNarrate(j) {
  ...
  const g = currentGuest(st);
  const glowKind = g.ryuwei ? "ryuwei" : g.lamu ? "lamu" : g.keluo ? "keluo" : g.ganqu ? "ganqu" : null; // 余味横向流光 / 拉姆圣光自上而下
...
async function doServe(sel) {
  ...
  const glowKind = g.ryuwei ? "ryuwei" : g.lamu ? "lamu" : g.keluo ? "keluo" : g.ganqu ? "ganqu" : null; // 余味/拉姆的菜 · 端菜与品尝全部流光炫彩
  ...
  note("出餐", reactNote || `给${g.name}上${[dish?.name, ...snackNames].filter(Boolean).map(n => `「${n}」`).join("、") || "「菜」"}，满意度${score}，好感+${d}。`);
...
async function doExpedition(node, intent) {
  ...
  const specialOf = (t) => (t || "").includes("余味") ? "ryuwei" : (t || "").includes("拉姆") ? "lamu" : (t || "").includes("珂萝") ? "keluo" : (t || "").includes("绀曲") ? "ganqu" : null;
...
async function rollFreshEvents(st) {
  ...
  const cands = GUESTS.filter(g => (!g.ryuwei || Math.random() < 0.25) && (!g.lamu || Math.random() < 0.25) && (!g.keluo || Math.random() < 0.25) && (!g.ganqu || Math.random() < 0.25)).sort(() => Math.random() - 0.5).slice(0, 2 + Math.floor(Math.random() * 2));
...
async function showPendingGifts() {
  ...
  const plain = pg.givers.filter(g => !g.ryuwei && !g.lamu && !g.keluo && !g.ganqu).map(row);
  const glow = pg.givers.filter(g => g.ryuwei || g.lamu || g.keluo || g.ganqu).map(g => ({ line: row(g), kind: g.ryuwei ? "ryuwei" : g.lamu ? "lamu" : g.keluo ? "keluo" : "ganqu" })); // 余味/拉姆/珂萝/绀曲的礼单 · 各用各的流光炫彩
...
function bind() {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    const map = { c: "cook", u: "snack", w: "brew", x: "serve", y: "world", t: "shop", m: "exp", n: "next", j: "invite", k: "sit", i: "bag", f: "settings", l: "trace", p: "notes", q: "save", h: "help" };
